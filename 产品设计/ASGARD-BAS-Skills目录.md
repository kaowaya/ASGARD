...
```
├── L0 基础模型层Skills（Foundation Models Tier）
│   ├── 机器学习类（20个Skills）
│   │   ├── F0.1 降维算法（PCA、t-SNE、UMAP）
│   │   ├── F0.2 聚类算法（K-Means、DBSCAN、GMM）
│   │   ├── F0.3 线性回归（LR、Ridge、Lasso）
│   │   ├── F0.4 支持向量机（SVM、SVR）
│   │   ├── F0.5 随机森林（RF）
│   │   ├── F0.6 XGBoost（xgboost）
│   │   ├── F0.7 LightGBM（lightgbm）
│   │   ├── F0.8 隐马尔可夫模型（HMM）
│   │   ├── F0.9 高斯过程回归（GPR）
...
│   │   ├── F0.13 自组织映射（SOM）
│   │   ├── F0.14 AdaBoost（adaboost）
│   │   ├── F0.15 Gradient Boosting（gbdt）
│   │   ├── F0.16 Extra Trees（extra trees）
│   │   ├── F0.17 Elastic Net（elastic net）
│   │   ├── F0.18 核方法（Kernel Ridge）
│   │   ├── F0.19 KNN（k-nearest neighbors）
│   │   └── F0.20 决策树（Decision Tree）
│   ├── 深度学习类（10个Skills）
│   │   ├── F0.21 CNN（卷积神经网络）
│   │   ├── F0.22 LSTM（长短期记忆网络）
│   │   ├── F0.23 Transformer（注意力机制）
│   │   ├── F0.24 GNN（图神经网络）
│   │   ├── F0.25 VAE（变分自编码器）
│   │   ├── F0.26 GAN（生成对抗网络）
│   │   ├── F0.27 GRU（门控循环单元）
│   │   ├── F0.28 TCN（时序卷积网络）
│   │   ├── F0.29 Seq2Seq（序列到序列）
│   │   └── F0.30 Attention（注意力机制）
│   ├── 强化学习类（5个Skills）
│   │   ├── F0.31 DQN（深度Q网络）
│   │   ├── F0.32 PPO（近端策略优化）
│   │   ├── F0.33 GRPO（群体相对策略优化）
│   │   ├── F0.34 A3C（异步优势演员-评论家）
│   │   └── F0.35 SAC（软演员-评论家）
│   └── 最优化类（8个Skills）
...
### 新增内容
1. **L0基础模型层**：新增43个基础算法Skills，覆盖机器学习、深度学习、强化学习、最优化四大类
2. **L2 BMS层级**：
   - SOC估计新增4种方法（安时积分、查表修正、EKF、AEKF、UKF）
   - SOH估计新增2种方法（两点法、AEKF）
   - 压力监测改为膨胀力监测，不再局限于固态电池
3. **L3云端层级**：
   - 香农熵改名为"安全熵"
   - 新增深度学习SOH估计
   - 析锂检测新增4种方法（弛豫法、SV-RV、P2D、PINN）
   - 内短路诊断新增3种方法（等效电路、P2D、PINN）
   - 新增容量异常检测（充高放低、一致性评价）
   - ICA从BMS移至云端
   - 新增云端智能均衡
4. **L4应用层级**：新增3个储能场站应用Skills（智能巡检、大屏、运维报告）
...
- 参数辨识：RLS、PSO
- SOH估计：ICA、SOH初始值猜测
- 故障诊断：析锂检测（弛豫电压法）、香农熵（安全熵）
- RUL预测：非时序特征工程、CNN-LSTM
- EIS分析：阻抗谱处理、霍夫变换、等效电路拟合
...
|------|---------|------|
| 已知类别数 | K-Means | 简单高效 |
| 未知类别数、含噪声 | DBSCAN | 自动发现类别 |
| 软聚类(概率输出) | GMM | 提供后验概率 |

---

#### F0.3-F0.7 监督学习类Skills
...
**随机森林（F0.5）**:
- **用途**: SOH/RUL预测、特征重要性排序
- **优势**: 鲁棒性强、可处理缺失值

**XGBoost（F0.6）**:
- **用途**: SOH集成学习、Kaggle竞赛首选
- **优势**: 精度高、自动处理缺失值
...
|-----|---|
| **算法类型** | 深度学习 - 图结构 |
| **核心架构** | Graph Conv + Message Passing |
| **输入数据** | 电池包图结构(节点=电芯,边=连接) |
| **输出结果** | 节点嵌入 / 图级别分类 |
| **应用场景** | 电池包一致性分析、热失控传播预测 |
| **电池类型复用** | ✅ 100%通用 |
| **部署位置** | 云端 |
...
4. 策略更新: ∇J ≈ ∑(advantage · ∇log π(a|s))
```

---

### F0.36-F0.43 最优化类Skills
...
|-----|---|
| **算法类型** | 基础积分法 |
| **核心公式** | SOC(k) = SOC(k-1) + I(k)·Δt/Qn/3600 |
| **输入数据** | 电流I、时间Δt、额定容量Qn |
| **输出结果** | SOC |
| **精度** | 依赖电流传感器精度 |
| **累积误差** | 需定期OCV校正 |
| **电池类型复用** | ✅ 100%通用 |
...
|-----|---|
| **算法类型** | 查表法 + 插值 |
| **核心原理** | 基于OCV-SOC映射表,通过电压反推SOC |
| **输入数据** | 电压V、温度T |
| **输出结果** | SOC |
| **精度** | ±3% |
| **电池类型复用** | ✅ 100%通用(需标定OCV曲线) |
| **部署位置** | BMS板端 |
...
```
1. 预测步骤(时间更新):
   SOC_k^- = SOC_{k-1} - I_k·Δt/Qn
   P_k^- = P_{k-1} + Q

2. 更新步骤(测量更新):
   K_k = P_k^-·H^T·(H·P_k^-·H^T + R)^(-1)
   SOC_k = SOC_k^- + K_k·(V_meas - V_est)
   P_k = (I - K_k·H)·P_k^-
```
...
**双自适应机制**:
1. **状态自适应**: SOC估计
2. **参数自适应**: R0、C1、C2在线辨识

**Sage-Husa自适应**:
```python
# 动态调整过程噪声Q和测量噪声R
Q_k = (1 - d_k)·Q_{k-1} + d_k·K_k·ε_k·K_k^T
R_k = (1 - d_k)·R_{k-1} + d_k·ε_k·ε_k^T
```
...
# 2n+1个Sigma点
χ_0 = x (均值)
χ_i = x + (√((n+λ)·P))_i  (i=1..n)
χ_{i+n} = x - (√((n+λ)·P))_i  (i=1..n)
...
**状态方程**:
```
X = [Q_loss, Uocv, U1, U2]^T
Q_loss(k+1) = Q_loss(k) + f(I_k, T_k, Δt)
```

**日历老化建模**:
```python
# 基于时间的自然衰减
calendar_degrad_index = single_circle_degrad / circle_working_time
...
# 大电流加速老化
if I_k > I_max:
    cycle_loss = (160 - I_k) * 70
    Q_loss += (1 - 0.95) / 700 * cycle_loss
```
...
**膨胀力来源**:
1. **液态电池**:SEI膜增厚、活性物质膨胀
2. **固态电池**:固-固界面接触失效、锂枝晶生长
...
离散化故障等级：
    ...
    ↓
香农熵计算：
    Safety_Entropy = -Σ p(Level) * log2(p(Level))
    ↓
系统有序度评估：
    Safety_Entropy ↓ → 系统有序 → 正常
    Safety_Entropy ↑ → 系统混乱 → 异常
```

**关键创新**：
- 安全熵： renamed from 香农熵（Shannon Entropy）
- 不直接使用易波动的原始差值
- 通过改进Z-分数转化为离散故障等级
- 有效过滤电流波动和环境噪声
...
**预警阈值**：
- SV-RV评分 > 70：高风险，建议降低充电电流
- SV-RV评分 > 85：极高风险，立即停止充电
...
**损失函数**：
```
Loss = Loss_data + λ_pde * Loss_pde
其中：
Loss_data = (Φ_pred - Φ_true)²
Loss_pde = PDE残差的L2范数
```
...
```
多目标函数:
  J = w1·寿命损失 + w2·不均衡度 + w3·能量损失 + w4·成本
...
|-----|---|
| **算法类型** | 计算机视觉 + 异常检测 |
| **核心模型** | 目标检测 + 热成像分析 + 时序预测 |
| **输入数据** | 巡检机器人图像、热成像、设备运行数据 |
| **输出结果** | 缺陷识别、风险预警、巡检报告 |
| **检测精度** | 缺陷识别率 >95% |
| **电池类型复用** | ✅ 100%通用 |
| **部署位置** | 边缘端(机器人) + 云端(分析) |
...
|-----|---|
| **算法类型** | 数据可视化 + 实时监控 |
| **核心技术** | WebSocket + 时序数据库 + BI工具 |
| **输入数据** | 实时电池数据、设备状态、告警信息 |
| **输出结果** | 可视化大屏、移动端推送 |
| **刷新频率** | 毫秒级实时更新 |
| **电池类型复用** | ✅ 100%通用 |
| **部署位置** | 本地部署 + 云端备份 |
...
**技术栈**：
- 前端：Vue3 + ECharts + WebGL
- 后端：FastAPI + WebSocket
- 数据库：InfluxDB(时序) + PostgreSQL(关系型)
...
|-----|---|
| **算法类型** | 自然语言处理 + 数据分析 |
| **核心模型** | 模板生成 + 数据洞察 + 自动化归档 |
| **输入数据** | 电池运行数据、巡检记录、故障日志 |
| **输出结果** | 周报/月报/年报 |
| **生成时间** | 分钟级(vs 传统数小时) |
| **电池类型复用** | ✅ 100%通用 |
| **部署位置** | 云端SaaS |
